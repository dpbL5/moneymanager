...
public class TransactionDao implements DaoInterface<TransactionModel>{
    ...
    public Map<String, Map<String, Double>> getTransactionSummaryByKind(String username, LocalDate startDate, LocalDate endDate) {
        String sql = "SELECT transaction_kind, type, SUM(amount) AS total " +
                "FROM transaction " +
                "WHERE username = ? AND transaction_date BETWEEN ? AND ? " +
                "GROUP BY transaction_kind, type";
        ...
        try (Connection connection = JDBCUtil.getConnection();
            ...
            while (resultSet.next()) {
                ...
                // Tạo map lồng nhau cho từng loại giao dịch
